...
void SharedBuffer::acquire() const {
    OSAtomicIncrement32Barrier(&mRefs);
}

int32_t	messydec( volatile int32_t *__theValue );
int32_t messydec( volatile int32_t *__theValue ){
    return OSAtomicDecrement32Barrier((int32_t*)__theValue)+1;
}

int32_t SharedBuffer::release(uint32_t flags) const
{
    int32_t prev = 1;
    if (onlyOwner() || ((prev = messydec(&mRefs)) == 1)) {
        mRefs = 0;
